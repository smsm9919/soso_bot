...
def main_bot_loop():
    ...
    while True:
        try:
            ...
            # ===== تحسين شروط الدخول =====
            # 1. فلترة الاتجاه العام باستخدام EMA 200
            # 2. شروط RSI لتأكيد الزخم
            # 3. شروط ADX لتأكيد قوة الترند
            # 4. شروط Supertrend لتأكيد الاتجاه
            # 5. تقاطع المتوسطات المتحركة (SMA)
            
            ma_cross_up = (sma_3 > sma_5 > sma_7 and 
                          current_price > ema_200_value and
                          adx_value > 25 and
                          rsi_value > 55 and
                          current_supertrend > 0)
            
            ma_cross_down = (sma_3 < sma_5 < sma_7 and 
                            current_price < ema_200_value and
                            adx_value > 25 and
                            rsi_value < 45 and
                            current_supertrend < 0)
            ...
            if not position_open:
                ...
                if current_time - last_trade_time < COOLDOWN_PERIOD:
                    ...
                    if ma_cross_up and price_range > 1.5:
                        # تأكيد نسبة الربح/مستوى المخاطرة
                        ...
                        if tp_percent >= MIN_TP_PERCENT:  # نسبة ربح لا تقل عن 0.75%
                            if last_direction == "BUY":
                                print("🚫 Last trade was also BUY — skipping repeated direction")
                            else:
                                print(colored("\n🚀 MA CROSS BUY SIGNAL (Improved Strategy)", "green", attrs=["bold"]))
                        ...
                        if tp_percent >= MIN_TP_PERCENT:  # نسبة ربح لا تقل عن 0.75%
                            if last_direction == "SELL":
                                ...
                                print(colored("\n🚀 MA CROSS SELL SIGNAL (Improved Strategy)", "red", attrs=["bold"]))
